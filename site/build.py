...
def build_signin() -> None:
    """The front door. Two ways in, both verified by the gateway: an email, or a
  ...
  <section class="authbox">
    ...
    <p class="auth-sub">Sign in with an email or a wallet. Either one mints your key.</p>

    <button type="button" class="auth-row primary" data-method="email">
      <span>Continue with email</span><span class="arrow">&rarr;</span></button>
    <button type="button" class="auth-row" data-method="wallet">
      <span>Continue with an Arc wallet</span><span class="arrow">&rarr;</span></button>

    <p class="auth-foot">Your key is the account. There is no profile behind it, and
  ...
  <noscript>
    <p class="note">Signing in needs JavaScript, because every method here is a live
      exchange with the gateway. You can still create a key without one:
      <code>curl -X POST {API}/keys</code>.</p>
  </noscript>
</main>"""
    write(OUT / "signin" / "index.html", shell(
        title=f"Sign in · {BRAND}", description=(
            "Sign in with an email or an Arc wallet. No profile, no account to fill in."),
